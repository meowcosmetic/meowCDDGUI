...
class _AddChildSheetState extends State<AddChildSheet> {
  ...
  final TextEditingController _specialMedicalConditions =
  ...
  final TextEditingController _earlyInterventionDetails =
      TextEditingController();
  final TextEditingController _familyDevelopmentalIssues =
      TextEditingController();
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _field(
                      ...
                      _specialMedicalConditions,
                    ),
                    _field('Chi tiết can thiệp sớm', _earlyInterventionDetails),
                    _field(
                      'Vấn đề phát triển gia đình',
                      _familyDevelopmentalIssues,
                    ),

                    LanguageDropdown(
                      value: _primaryLanguage,
                      onChanged: (value) => setState(
                        () => _primaryLanguage = value ?? 'Vietnamese',
                      ),
                    ),
                    ...
                    _buildCheckbox(
                      'Sinh non',
                      _isPremature,
                      (value) => setState(() => _isPremature = value),
                    ),
                    _buildCheckbox(
                      'Có can thiệp sớm',
                      _hasEarlyIntervention,
                      (value) => setState(() => _hasEarlyIntervention = value),
                    ),
        ...
        children: [
          ...
          Container(
            ...
            child: DropdownButtonFormField<String>(
              ...
              items: diagnosisOptions.entries
                  .map(
              ...
              onChanged: (value) => setState(
                () =>
                    _developmentalDisorderDiagnosis = value ?? 'NOT_EVALUATED',
              ),
            ),
          ),
        ],
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      final childData = ChildData(
        ...
        primaryLanguage:
            LanguageDropdown.languageOptions[_primaryLanguage] ?? 'Tiếng Việt',
        ...
        gestationalWeek: _gestationalWeek.text.trim().isNotEmpty
            ? int.tryParse(_gestationalWeek.text.trim())
            : null,
        birthWeightGrams: _birthWeightGrams.text.trim().isNotEmpty
            ? int.tryParse(_birthWeightGrams.text.trim())
            : null,
        earlyInterventionDetails:
            _earlyInterventionDetails.text.trim().isNotEmpty
            ? _earlyInterventionDetails.text.trim()
            : null,
        status: 'ACTIVE',
      );
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        throw Exception(
          'Failed to add child: ${response.statusCode} - ${response.body}',
        );
      }
    } catch (e) {
      if (!mounted) return;
